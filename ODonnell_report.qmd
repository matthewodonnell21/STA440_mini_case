...
With a $p < \alpha = 0.05$, the model results show a statistically significant negative relationship between tenure and satisfaction with Durham. In fact, it is the only model covariate with a statistical significant effect (although multicollinearity between inputs like $income$, $housing$, and $own$ might be diluting each others' effects). The results tell us that for respondents who have identical respondents for all other inputs (same income bracket, same race, etc.), each additional year of living in Durham decreases the estimated log-odds being satisfied with Durham decreases by roughly 0.015. 

The main implication of these results is that in a rapidly changing metropolitan area, those who have lived in Durham longer might be feeling more of the growing pains. In future urban planning, it will be important to include voices of tenured residents in the decision making. 
